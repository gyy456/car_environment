...
#include <mecanum_drive_controller/gazebo_ros_mecanum_drive.h>
...
namespace gazebo
{
  ...
  // Load the controller
  void GazeboRosMecanumDrive::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf)
  {
    ...
    {
      ROS_INFO("MecanumDrivePlugin missing <robotNamespace>, "
    ...
    {
      ROS_WARN("MecanumDrivePlugin (ns = %s) missing <commandTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN("MecanumDrivePlugin (ns = %s) missing <odometryTopic>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN("MecanumDrivePlugin (ns = %s) missing <odometryFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN("MecanumDrivePlugin (ns = %s) missing <robotBaseFrame>, "
          "defaults to \"%s\"",
    ...
    {
      ROS_WARN("MecanumDrivePlugin (ns = %s) missing <odometryRate>, "
          "defaults to %f",
    ...
    {
      ROS_WARN("MecanumDrivePlugin (ns = %s) missing <cmdVelTimeOut>, "
          "defaults to %f",
    ...
    if (!sdf->HasElement("publishOdometryTf")) {
      ROS_WARN("MecanumDrivePlugin Plugin (ns = %s) missing <publishOdometryTf>, defaults to %s",
    ...
    {
      ROS_FATAL_STREAM("MecanumDrivePlugin (ns = " << robot_namespace_
    ...
    callback_queue_thread_ =
      boost::thread(boost::bind(&GazeboRosMecanumDrive::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_ =
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&GazeboRosMecanumDrive::UpdateChild, this));
  ...
  void GazeboRosMecanumDrive::cmdVelCallback(
  ...
  tf::Transform GazeboRosMecanumDrive::getTransformForMotion(double linear_vel_x, double angular_vel, double timeSeconds) const
  {
    tf::Transform tmp;
    tmp.setIdentity();
